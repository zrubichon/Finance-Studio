...
iexport default function InterviewStudio()ack: Track) {
  ...
  return (
    <div className="workspace-stack">
      <section className="control-panel interview-control-panel">
        <div>
          ...
          <div className="chip-row">
            {interviewTracks.map((item) => <button className={track === item.key ? "chip active" : "chip"} onClick={() => setTrack(item.key)} key={item.key} type="button">{isFrench ? item.fr : item.key}</button>)}
          </div>
        </div>
      ...
      <section className="interview-question-panel">
        <div className="question-meta"><span className="mini-label">{text("TECHNICAL / MARKET QUESTION", "QUESTION TECHNIQUE / MARCHÉ")}</span><span className="pill accent">{isFrench ? interviewTracks.find((item) => item.key === track)?.fr : track}</span></div>
